...
def parse_args():
    parser = argparse.ArgumentParser(description='DescNet eval tool for grid datasets.')
    ...
    parser.add_argument('-d', '--database', 
                        help='if from_database is true this is the input descriptor database file, otherwise it is the folder containing the dataset.', 
                        required=True)    
...
if __name__ == "__main__":
    ...
    test_loader = make_data_loader(args.test_dataset_dir, 1, 'test', data_cfg, network_cfg['n_clusters'])
    ...
    for batch in tqdm.tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            #visualize_utils.show_maps(batch['anchor'][0].detach().cpu().numpy(), maps.detach().cpu().numpy(), max_nmaps=64)
